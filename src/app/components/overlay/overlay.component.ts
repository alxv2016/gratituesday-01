...
export class OverlayComponent implements AfterViewInit, OnDestroy {
  ...
  @ViewChild('overlayHeader') overlayHeader!: ElementRef;
  ...
  private onTransitionEnd(ev: TransitionEvent, destroy: boolean = false) {
    ...
    requestAnimationFrame(() => {
      this.render.removeClass(this.host, 'c-overlay--animate');
  ...
  private checkScrollbar(): void {
    ...
    const scrollable = this.checkScroll(this.overlayWindow.nativeElement, this.overlayContent.nativeElement);
    if (scrollable) {
      this.render.setStyle(this.overlayHeader.nativeElement, 'width', `calc(100% - ${scrollbarWidth - 7}px)`);
    }
  }
  ...
  private createComponent(): void {
    ...
    this.childComponent.changeDetectorRef.detectChanges();
    requestAnimationFrame(() => {
      this.render.addClass(this.host, 'c-overlay--animate');
      this.render.addClass(this.host, 'c-overlay--visible');
  ...
  closeBottomSheet() {
    this.render.addClass(this.host, 'c-overlay--animate');
    this.render.removeClass(this.host, 'c-overlay--visible');
  ...
  ngAfterViewInit(): void {
    ...
    this.zone.runOutsideAngular(() => {
      ...
      this.clickEventHandlers.push(
        ...
        this.render.listen(this.overlayWindow.nativeElement, 'click', (e: MouseEvent) => e.stopPropagation())
      );
    });
  }
